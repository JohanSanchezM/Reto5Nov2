...
import jsm.reto4.reto4.Services.ClienteServicio;
...
public class ClienteControlador {
    ...
    public Optional<Cliente>getClient(@PathVariable("id")int clientId){
        return clienteServicio.getClient(clientId);
    }
    @PostMapping("/save")
    @ResponseStatus(HttpStatus.CREATED)
    public Cliente save(@RequestBody Cliente client){
        return clienteServicio.save(client);
    }
    @PutMapping("/update")
    @ResponseStatus(HttpStatus.CREATED)
    public Cliente update(@RequestBody Cliente client) {
        return clienteServicio.update(client);
    }
    @DeleteMapping("/{id}")
    @ResponseStatus(HttpStatus.NO_CONTENT)
    public boolean delete(@PathVariable("id") int clientId) {
        return clienteServicio.deleteClient(clientId);
    }
